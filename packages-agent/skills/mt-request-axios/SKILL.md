...
2. 检查目标项目是否已经使用 axios，以及是否有现存请求封装。
3. 确认安装 `@mt-kit/request-axios`，外部项目从包名导入，不引用源码路径。
4. 创建 `RequestClient` 实例，明确 `baseURL`、`timeout`、`responseReturn`、`paramsSerializer`。
5. 按业务响应格式接入 `defaultResponseInterceptor`。
6. 按认证需求接入 `authenticateResponseInterceptor`。
7. 按 UI 提示机制接入 `errorMessageResponseInterceptor`。
8. 对上传下载场景分别使用 `upload` 和 `download`，不要混用普通 JSON 请求配置。

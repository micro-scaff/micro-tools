...
cp -R /path/to/micro-tools/packages-agent/skills/mt-dev-configs .agents/skills/
```
...
cp -R /path/to/micro-tools/packages-agent/skills/mt-dev-configs .claude/skills/
```

复制后，外部项目里应该能看到这样的结构：

```text
.agents/skills/mt-dev-configs/
├── SKILL.md
├── agents/
│   └── openai.yaml
└── references/
...
```text
请阅读 /path/to/micro-tools/packages-agent/skills/mt-dev-configs/SKILL.md，
...
```text
使用 $mt-dev-configs 检查当前项目的 ESLint 接入。
```

或者：

```text
/mt-dev-configs 检查当前项目的 ESLint 接入。
```
...
- `name`：必填。skill 名称，使用小写字母、数字和连字符，例如 `mt-dev-configs`。
...
```yaml
interface:
  display_name: "MT Dev Configs"
  short_description: "帮助内部和外部项目接入、迁移、组合和排查 MT 开发配置包"
  default_prompt: "使用 $mt-dev-configs 为这个项目接入或排查 MT 开发配置包。"
...
`mt-dev-configs` 是一个“参考知识 + 接入流程”类 skill：

- `SKILL.md`：触发说明、项目类型判断和主流程。
- `references/package-map.md`：MT 开发配置包的包名、入口和用途。
- `references/eslint-config-map.md`：`@mt-kit/eslint-config` 的内部模块映射。
- `references/eslint-integration-recipes.md`：外部项目安装和 ESLint 配置模板。
- `references/troubleshooting.md`：跨工具常见问题分类排查。
...
下面以本仓库的 `mt-dev-configs` 为例。其他 skill 只需要把目录名换成对应名称，例如 `mt-vue-packages`、`mt-react-packages`、`mt-request-axios`。

### Codex

常见用法：

- 显式调用：在提示词中写 `$skill-name`，例如 `$mt-dev-configs`。
...
```bash
mkdir -p .agents/skills
cp -R packages-agent/skills/mt-dev-configs .agents/skills/
```

使用示例：

```text
使用 $mt-dev-configs 帮我检查当前项目的 ESLint 接入是否正确。
```

也可以不显式写 `$mt-dev-configs`，直接描述任务：
...
```text
使用 $mt-dev-configs 排查这个项目里 import 排序和 React lint 规则的问题。
```
...
```bash
mkdir -p .claude/skills
cp -R packages-agent/skills/mt-dev-configs .claude/skills/
```

使用示例：

```text
/mt-dev-configs 检查当前项目的 ESLint 配置，并给出需要修改的文件。
```
...
```text
/mt-dev-configs 排查这个项目里的 ESLint peer dependency 和 flat config 问题。
```
...
```text
/plugin-name:mt-dev-configs 帮我接入 @mt-kit/eslint-config。
```
...
```text
用户请求：
使用 $mt-dev-configs 检查这个外部 Vue 项目的 ESLint 接入问题。

推荐执行：
1. 主 Agent 读取 mt-dev-configs 的 SKILL.md。
2. 读取 references/eslint-integration-recipes.md 判断 Vue 项目配置。

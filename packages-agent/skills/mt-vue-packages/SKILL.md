...
1. 检查目标项目是否是 Vue 3，是否使用 Vite、Element Plus、ECharts 等相关依赖。
2. 根据需求选择最小包，不一次性安装所有 Vue 包。
3. 确认外部项目安装的是 registry 包，不引用本仓库 `packages-vue/*` 源码路径。
4. 按 reference 中的导出表选择正确 API。
5. 若涉及组件或交互行为，优先参考 README、stories 和 `src/index.ts`。
6. 排错时先看 peer/dependency、样式依赖、自动导入、构建产物和类型导出。
